...
| **Results dashboard** | Summary statistics, per-case breakdown, and a full leaderboard — all computed live from the database |
...
| Image drawing | HTML5 Canvas API (no JS framework — vanilla JS only) |
| Form handling | python-multipart 0.0.9 |
| File serving | aiofiles 24.1 |
| Python | 3.14+ (uses `Optional[T]` typing, not `T \| None`) |
...
│   └── routes/
│       ├── __init__.py
│       ├── cases.py       # All GET page routes + image endpoint + results
...
│   ├── evaluate.html      # Single-output rating form + interactive canvas
│   ├── compare.html       # Head-to-head comparison form + read-only canvas
│   └── results.html       # Statistics dashboard + Elo leaderboard
├── data/
│   └── db.sqlite          # SQLite database (auto-created on first boot, persisted via Docker volume)
├── Dockerfile
├── docker-compose.yml
├── requirements.txt
└── README.md
...
If image downloads fail (e.g. no network access), the cases and outputs are still fully seeded — only the image BLOBs will be missing. The "View images ↗" link to the Radiopaedia viewer will still work.
...
- Iterates over a `CASES` list of 6 Radiopaedia cases (title, modality, clinical prompt, patient age/sex, image URL, Radiopaedia viewer URL)
...
All page navigation is handled by GET routes in `cases.py`. All form submissions are handled by POST routes in `evaluations.py`. After a successful POST, the server redirects back to the home page (`303 See Other`) so the browser does not re-submit the form on refresh.
...
Home (/)
  ...
  → fill in ratings + optionally draw regions + submit
...
```
Home (/)
  → click "Compare ↔" on a case
  → GET /cases/{id}/compare     — comparison form with auto-selected pair
...
The server uses `itertools.combinations(outputs, 2)` to enumerate every possible pair of outputs for the case. For each pair it queries `ComparisonEvaluation` for the count of times that exact pair has been compared (in either direction — `a→b` or `b→a`). The pair with the lowest count is shown. If multiple pairs are tied, the first is used. This guarantees balanced coverage across all pairs as comparisons accumulate.
...
A `ComparisonEvaluation` row is then written and the server redirects to `/`.
...
| Cases complete | Cases where every model output has ≥1 evaluation |
| Flagged outputs | Count of evaluations with `is_flagged = true` |
| Queue remaining | Outputs with zero evaluations |
| Total comparisons | Count of `ComparisonEvaluation` rows |

**Per-output statistics:**

For each case, each model output gets a row showing:
- Number of evaluations received
- Average overall quality rating (1–5)
- Average accuracy, completeness, and clarity ratings
- Number of flags

Averages skip `NULL` values (unanswered optional dimensions), so they always reflect only the evaluators who answered that dimension.
...
|---|---|---|---|
| GET | `/` | `cases.index` | Home page — case list with evaluation counts |
...
| GET | `/cases/{id}/compare` | `cases.compare_page` | Head-to-head form — auto-selects least-compared pair |
| GET | `/cases/{id}/image` | `cases.case_image` | Serves image BLOB with `Content-Type: image/jpeg` |
| GET | `/results` | `cases.results_page` | Statistics + Elo leaderboard |
...
All POST routes redirect to `/` with HTTP 303 on success.
...
- **`model_output.status`** is flipped to `"evaluated"` on the *first* single-output evaluation only. It is not affected by comparison evaluations. It is used for display purposes (queue count on the results page) but does not gate access to the form.
